...
* Tail Call Elimination
  ...
  To support deep recursion without overflowing the Python call stack, Hornet uses
  a small trampoline system.

** Design

   A =tailcall= decorator marks recursive continuations.  The =trampoline= driver
   repeatedly calls thunks until no further continuation remains.

** Implementation
#+BEGIN_SRC python :tangle hornet/tailcalls.py
