...
* State Monad
  ...
  We also need a mechanism for state threading through computations.  This monad
  abstracts explicit state passing and provides combinators for reading,
  modifying, and writing state.

** Implementation
#+BEGIN_SRC python :tangle hornet/states.py
