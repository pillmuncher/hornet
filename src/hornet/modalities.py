...
@predicate(kleisli(AccessPred1, AccessPred2))
def _kleisli(db: Database, subst: Subst) -> Step[Database, Environment]:
...
def modal(db: Database) -> Database:
    child = db.new_child()
    child.tell(_necessarily, _possibly, _epistemic_world, _compliance_world, _kleisli)
    return child
