...
class Expression[T: Term]:
    ...
    def __neg__(self):
        return Expression(terms.USub(promote(self)))

    def __pos__(self):
        return Expression(terms.UAdd(promote(self)))

    def __invert__(self):
        return Expression(terms.Invert(promote(self)))

    def __add__(self, other):
        rhs = other.term if isinstance(other, Expression) else other
        return Expression(terms.Add(promote(self), rhs))

    __radd__ = flip(__add__)

    def __sub__(self, other):
        return Expression(terms.Sub(promote(self), promote(other)))

    __rsub__ = flip(__sub__)

    def __mul__(self, other):
        return Expression(terms.Mult(promote(self), promote(other)))

    __rmul__ = flip(__mul__)

    def __truediv__(self, other):
        return Expression(terms.Div(promote(self), promote(other)))

    __rtruediv__ = flip(__truediv__)

    def __floordiv__(self, other):
        return Expression(terms.FloorDiv(promote(self), promote(other)))

    __rfloordiv__ = flip(__floordiv__)

    def __mod__(self, other):
        return Expression(terms.Mod(promote(self), promote(other)))

    __rmod__ = flip(__mod__)

    def __pow__(self, other):
        return Expression(terms.Pow(promote(self), promote(other)))

    __rpow__ = flip(__pow__)

    def __lshift__(self, other):
        return Expression(terms.LShift(promote(self), promote(other)))

    __rlshift__ = flip(__lshift__)

    def __rshift__(self, other):
        return Expression(terms.RShift(promote(self), promote(other)))

    __rrshift__ = flip(__rshift__)

    def __and__(self, other):
        return Expression(terms.BitAnd(promote(self), promote(other)))

    __rand__ = flip(__and__)

    def __xor__(self, other):
        return Expression(terms.BitXor(promote(self), promote(other)))

    __rxor__ = flip(__xor__)

    def __or__(self, other):
        return Expression(terms.BitOr(promote(self), promote(other)))
